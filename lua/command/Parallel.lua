...
local function parActFin(self)
  local fin = true
  for i=1,#self.actions do
    fin = not self.actionState[i] and fin
  end
  return fin
end
...
local function isInterruptible(self)
  ...
  for i=1,#self.actions do
    if self.actionState[i] and not self.actions[i]:IsInterruptible() then
      return false
    end
  end
  return true
